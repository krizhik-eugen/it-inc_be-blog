...
describe('Posts Controller', () => {
    ...
    describe('POST /posts', () => {
        it('creates a new post', async () => {
            const response = await request(app).post(baseRoutes.posts).send(testPost);
        ...
        it('returns an error if required fields are missing', async () => {
            ...
            const response = await request(app).post(baseRoutes.posts).send(newPost);
    ...
    describe('GET /posts/:id', () => {
        it('returns a post by id', async () => {
            ...
            const response = await request(app).get(`${baseRoutes.posts}/${createdPost.id}`);
            expect(response.status).toBe(HTTP_STATUS_CODES.OK);
    ...
    describe('PUT /posts/:id', () => {
        it('updates a post', async () => {
            ...
            const response = await request(app).put(`${baseRoutes.posts}/${createdPost.id}`).send(updatedPost);
            expect(response.status).toBe(HTTP_STATUS_CODES.NO_CONTENT);
            const updatedPostInDB = await postsModel.getPost(createdPost.id);
            expect(updatedPostInDB).toEqual(expect.objectContaining(updatedPost));
        });

        it('returns an error if post is not found', async () => {
            const updatedPost = { ...testPost, title: 'Updated Post' };
            const response = await request(app).put(`${baseRoutes.posts}/123`).send(updatedPost);
            expect(response.status).toBe(HTTP_STATUS_CODES.NOT_FOUND);
        });
    });
    ...
    describe('DELETE /posts/:id', () => {
        it('deletes a post', async () => {
            const createdPost = await postsModel.addNewPost(testPost);
            const response = await request(app).delete(`${baseRoutes.posts}/${createdPost.id}`);
            expect(response.status).toBe(HTTP_STATUS_CODES.NO_CONTENT);
        ...
        it('returns an error if post is not found', async () => {
            const response = await request(app).delete(`${baseRoutes.posts}/123`);
            expect(response.status).toBe(HTTP_STATUS_CODES.NOT_FOUND);
        });
    });
});