...
export const errorValidator = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (errors.length) {
        res.status(HTTP_STATUS_CODES.BAD_REQUEST).json({
            errorMessages: errors.map(error => ({ message: error.msg, field: error.type === 'field' ? error.path : '' }))
        })
        return;
    }
...
export const requestValidator = (bodySchema: Schema, paramSchema?: Schema,) => {
    ...
    const scopes: Parameters<typeof checkSchema>[1] = paramSchema ? ['body', 'params'] : ['body'];
